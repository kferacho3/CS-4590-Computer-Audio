...
void setup() {
  ...
  bd.addMessageListener(
    new Bead() {
      protected void messageReceived(Bead b) { beat = 4; }
    }
  );
...
void draw()
{
  ...
  // Find strongest frequency and comput avgFq - see 9.3
  
  // draw the average frequency on screen
  text(" Dectected Strongest Frequency: " + avgFq, 500, 100);

  if (f.getFeatures() != null && random(1.0) > .75) {
    float inputFrequency = f.getFeatures();
    if (inputFrequency < 3000) {
      avgFq = (.4 * inputFrequency) + (.6 * avgFq);
      frequencyGlide.setValue(avgFq);
    }
  }

  strongestFreqIndex = (int) ((avgFq / 19980.0) * 256.0);
